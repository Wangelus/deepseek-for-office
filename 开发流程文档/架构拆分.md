...
- [ ] `AbortController` 停止生成 — Phase 2：SSE 流式时须支持取消
- [ ] SSE 流式解析器 — Phase 2：Buffer 拼接 + JSON 断行处理（~80 行）
- [ ] 请求重试机制 — Phase 2：网络异常自动重试 1-2 次
- [ ] 请求队列并发控制 — Phase 2：长文本分段时限制 3-5 路并发
...
- [ ] 递归分割器实现 — 按 `\n\n` → `\n` → `。` → `；` → `，` 优先级递归
- [ ] 分段参数配置 — 默认 2000 字/段 + 200 字重叠（滑动窗口）
- [ ] 并行 Map 阶段 — `Promise.allSettled()` 3-5 路并发
- [ ] Reduce 合并阶段 — 排序 → 边界去重 → 层级摘要生成
- [ ] 上下文元信息注入 — 每段带 `[第 N/M 段，前一段摘要：...]`
- [ ] 长文本总结（层级摘要）— 一句话结论 → 三段要点 → 逐段摘要
- [ ] 长文本翻译 — 分段翻译 + 术语一致性保持
- [ ] 长文本纠错 — 分段检查 + 跨段一致性验证
- [ ] 进度指示器 — "正在处理第 3/10 段..."
...
- [ ] 扩写模式 — "扩展到约 N 字"，`max_tokens` 调高至 4096
- [ ] 略写/压缩模式 — "压缩到约 N 字，保留核心信息"，`temperature` 调低至 0.3
- [ ] 保持风格模式 — 传入原文前 200 字作为 few-shot 示例
- [ ] UI 字数目标选择器 — 滑块或输入框
- [ ] 扩写/略写结果对比 — 原文 vs 处理后字数对比展示
...
- [ ] `ReadableStream` 读取器 — `response.body.getReader()`
- [ ] Buffer 拼接策略 — 处理 TCP 分包导致的 JSON 截断
- [ ] 增量 DOM 更新 — 逐字追加到消息气泡（打字机效果）
- [ ] `AbortController` 停止 — 用户点击"停止生成"按钮
- [ ] 流式完成后 Markdown 渲染 — 完整文本交 markdown-it 渲染
- [ ] 流式异常处理 — 网络中断提示 + 保留已生成内容
- [ ] 自动重连 — 可选 `@microsoft/fetch-event-source`（~3KB）
...
- [ ] Skill YAML Schema 设计 — 已完成（name/version/system_prompt/document_types/few_shot/terminology/format_rules）
- [ ] js-yaml 集成 — 解析 YAML 为 JS 对象
- [ ] Skill YAML 校验 — 必填字段检查 + 格式合法性验证
- [ ] Skill 版本管理 — `version` 字段，升级时提示用户
...
- [ ] `compileSkillPrompt()` 核心函数 — 编译入口
- [ ] `{{#document_types}}` 占位符替换 — 匹配用户选择的文种模板
- [ ] `{{#few_shot_examples}}` 注入 — 最多 3 个示例注入为 few-shot
- [ ] `{{user_instruction}}` 注入 — 用户实际指令
- [ ] 领域词典 RAG 检索 — 从 terminology 列表匹配相关政治表述
- [ ] 编译缓存 — 同一 Skill + 相同参数 → 缓存结果
...
- [ ] `skills/` 目录加载 — fetch 本地 YAML 文件
- [ ] 用户自定义 Skill 存储 — 解析后缓存到 localStorage
- [ ] Skill 列表管理 UI — 查看/启用/禁用/删除
- [ ] Skill 导入（本地文件）— 选择 `.yml` 导入
...
- [ ] Skill 下拉菜单 UI — 分组显示（内置 Skill / 自定义 Skill）
- [ ] 文档类型二级选择 — 选公文后 → 选"通知/报告/请示/函/纪要"
- [ ] 当前 Skill 状态指示 — 显示激活的 Skill 名称和图标
